          ...
          {cycles.map(cycle => <details className="history-cycle" open={cycle.is_primary || cycle.id === selectedCycleId || comparisonMode} key={cycle.id}>
            ...
            {[cycle.baseline, ...(cycle.increments ?? [])].map(node => <div className={`history-node ${node.research_kind}`} key={node.id}>
              <button className="history-select" aria-current={selectedId === node.id ? "page" : undefined} onClick={() => onSelect(node.id)}>
                <time>{node.analysis_date}</time><span className="history-node-context"><span>{t(node.research_kind === "full" ? "fullResearch" : "incrementalResearch")}</span>
              ...
              {comparisonMode && <button className="button compact-button" title={t(selections.some(item => item.node_id === node.id) ? "removeFromComparison" : "selectForComparison")} aria-label={t(selections.some(item => item.node_id === node.id) ? "removeFromComparison" : "selectForComparison")} aria-pressed={selections.some(item => item.node_id === node.id)} disabled={selections.length === 2 && !selections.some(item => item.node_id === node.id)} onClick={() => onCompare?.(node)}><Icon name={selections.some(item => item.node_id === node.id) ? "check" : "compare"} /></button>}
