...
export default function ComparisonValue({ field, value, index, onEvidence }: {
  ...
  const prose = (text: unknown) => {
    ...
    for (const match of text.matchAll(/\bev_[a-f0-9]{12}\b/g)) aliases[match[0]] ??= t("evidenceReferenceUnavailable");
