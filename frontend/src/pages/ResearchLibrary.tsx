...
import type { TimelineDetail } from "../api/client";
    ...
    {!page ? (!error && <div className="loading" role="status">{t("loading")}</div>)
      : page.items?.length ? <div className="table-wrap"><table className="research-library-table">
        ...
        <tbody>{page.items.map(item => <Fragment key={item.instrument}><tr>
          ...
          <td>{item.primary_analysis_date ?? "—"}<small className="secondary-line">{t("baselineDate")}: {item.primary_baseline_date ?? "—"}</small></td><td><small>{t("latestResearch")}: {item.latest_analysis_date}</small><small className="secondary-line">{t("fullResearch")}: {item.full_cycle_count} · {t("incrementalResearch")}: {item.incremental_node_count ?? 0}</small><button className="text-button" aria-expanded={params.get("expanded") === item.instrument} onClick={() => update({ expanded: params.get("expanded") === item.instrument ? null : item.instrument, cycle_offset: null })}>{t("historyNavigation")}</button></td>
        </tr>{params.get("expanded") === item.instrument && <tr><td colSpan={4}><LibraryCycles instrument={item.instrument} offset={Math.max(0, Number(params.get("cycle_offset")) || 0)} onOffset={value => update({ cycle_offset: String(value) })} /></td></tr>}</Fragment>)}</tbody></table></div>
...
function LibraryCycles({ instrument, offset, onOffset }: { instrument: string; offset: number; onOffset: (offset: number) => void }) {
  const { t } = useTranslation();
  const navigate = useNavigate();
  ...
  useEffect(() => {
    ...
    api.timeline(instrument, 12, offset).then(value => { if (active) setData(value); }, cause => { if (active) setError(String(cause)); });
    return () => { active = false; };
  }, [instrument, offset, revision]);
