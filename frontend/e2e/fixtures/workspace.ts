import { capabilities as makeCapabilities, modelCatalog } from "./models";
...
export function workspaceFixture(locale = "en") {
  ...
  const older = { ...full, id: "older", research_rating: "Hold", request: { ...full.request, analysis_date: "2026-06-01" } };
  ...
  const capabilities = makeCapabilities(locale);
  return function respond(url: URL, method = "GET", body?: string | null) {
    ...
    if (path === "/api/v1/health") return { status: "ok", queue: { queued: 1, running: 1 } };
    if (path === "/api/v1/instruments/recent") return summaries;
    if (path.includes("/models")) return modelCatalog;
    ...
    if (path === "/api/v1/run-groups") return { items: [{ id: "full", kind: "cycle", instrument: "NVDA", baseline: full, is_primary: true, research_runs: [full, ...increments.map(node => ({ ...increment, id: node.id, request: { ...increment.request, analysis_date: node.analysis_date } }))], related_tasks: [pending], matched_run_ids: ["full", ...increments.map(node => node.id), "pending"], status_counts: { succeeded: 25, running: 1 } }, { id: "older", kind: "cycle", instrument: "NVDA", baseline: older, is_primary: false, cycle_warning: true, research_runs: [older], related_tasks: [failed], matched_run_ids: ["older", "failed"], status_counts: { succeeded: 1, failed: 1 } }, { id: "standalone", kind: "standalone", instrument: "4568.T", baseline: null, is_primary: false, research_runs: [], related_tasks: [standalone], matched_run_ids: ["standalone"], status_counts: { queued: 1 } }], total: 3, limit: 12, offset: 0 };
    if (path === "/api/v1/runs") { const task = url.searchParams.get("status") === "running" ? pending : url.searchParams.get("status") === "failed" ? failed : standalone; return { items: [task], total: 1, limit: 4, offset: 0 }; }
    ...
    if (path.startsWith("/api/v1/runs/")) { const id = path.split("/").at(-1)!; const task = [pending, failed, standalone].find(item => item.id === id); if (task) return { run: task, result: null, research_node: null, evidence_status: { status: "pending" } }; const node = increments.find(item => item.id === id) ?? (id === "older" ? { ...baseline, id: "older", cycle_id: "older", analysis_date: "2026-06-01", is_primary: false, is_cycle_head: true } : baseline); const isIncremental = node.research_kind === "incremental"; const run = isIncremental ? { ...increment, id, request: { ...increment.request, analysis_date: node.analysis_date } } : id === "older" ? older : full; return { run, result: { ...report, run_id: run.id, reports: isIncremental ? {} : report.reports }, research_node: node, evidence_status: { status: "sealed" }, incremental_context: isIncremental ? { analysis_brief: { markdown: `# ${copy.brief}\n\n${copy.reason}\n\n## ${copy.sections[0]}\n\n${copy.paragraph}\n\n## ${copy.sections[2]}\n\n${copy.risk}`, report_sections: [], evidence_refs: ["ev_0123456789ab"], warnings: [] }, full_baseline: { run_id: "full", analysis_date: baseline.analysis_date, decision: report.decision } } : null }; }
    return {};
  };
}
