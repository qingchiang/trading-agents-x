...
**Japanese-market data (this fork, `dataflows/jp/`).** `.T` tickers route to a JP vendor stack wired through the suffix-routing mechanism above (`data_vendors_by_market[".T"]` in `default_config.py`) — no upstream behavior changes when the map is empty. The stack is **live-first but never leaks future data**; date-safety helpers live in `jp/lookahead.py`. By category:
- **Prices & indicators** — `jquants.py` (J-Quants API **v2**, `x-api-key` auth; OHLCV via `/equities/bars/daily`) with `jquants_indicator.py`; yfinance stays a keyless fallback.
...
- **News** — `jp_news.py` is a multi-source **assembler** (the router is first-success fallback, so a plain chain can't keep sources side-by-side): EDINET statutory filings (`edinet_news.py`) + TDnet 適時開示 (`tdnet_news.py`) + Google-News-RSS media (`google_news.py`, company name resolved via `company_info.py` off J-Quants `/equities/master`). All are per-ticker; `get_global_news` stays market-agnostic.
- **Sentiment** — prefetched (the sentiment analyst does not tool-call): market-level investor flows (`jquants_sentiment.py`, `/markets/trades_spec`) + per-stock EDINET 大量保有 / 公開買付 TOB filings (`edinet_holdings.py`), with `yfinance_sentiment.py` as fallback. EDINET target-matching uses a ticker→EDINET-code map (`edinet_code_map.py`) that ships a committed seed snapshot and **self-heals** by learning issuer codes while scanning `documents.json`.
