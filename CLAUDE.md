...
- **Fundamentals** — two per-method **assemblers**: `jp_fundamentals.py` (J-Quants `/fins/summary` + date-safe computed valuation ratios, TOPIX-weekly beta, dual-mode forward) serves `get_fundamentals`; `jp_statements.py` (summary + curated yfinance line items) serves the three statement methods. Each assembler serves only its own methods, so the router picks the right one per method.
- **News** — `jp_news.py` is a multi-source **assembler** (the router is first-success fallback, so a plain chain can't keep sources side-by-side): EDINET statutory filings (`edinet_news.py`) + TDnet 適時開示 (`tdnet_news.py`) + Google-News-RSS media (`google_news.py`, company name resolved via `company_info.py` off J-Quants `/equities/master`). For `.T` tickers, News also receives J-Quants `/equities/investor-types` as explicitly market-level regional context: the as-of market section is resolved through `/equities/master` (`0111`/`0112`/`0113`), never defaulted to Prime, and must not be attributed to the ticker. `get_global_news` stays market-agnostic.
- **Sentiment** — prefetched (the sentiment analyst does not tool-call): per-stock J-Quants margin/short data, EDINET 大量保有 / 公開買付 TOB filings (`edinet_holdings.py`), and live-only analyst ratings. Market-wide investor flows are deliberately excluded. EDINET target-matching uses a ticker→EDINET-code map (`edinet_code_map.py`) that ships a committed seed snapshot and **self-heals** by learning issuer codes while scanning `documents.json`. US StockTwits/Reddit are skipped for historical trade dates and are date-window-filtered for near-live runs.
- **Macro** — cross-border and market-agnostic (US + JP pulled together): `fred.py`, `estat.py` (e-Stat), `boj.py`. All share `macro_common.SeriesCache`, an in-memory LRU plus an opt-in **cross-run disk layer** (namespaced fred/estat/boj only) that is look-ahead-safe by key, settle-gated with a grace window, TTL-refreshed, and file-count-bounded.
