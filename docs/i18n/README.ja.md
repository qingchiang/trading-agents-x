...
pending の結果には、固定された 5 取引間隔の観測期間を使用します。結果を確定するには、
...
期間を短縮せず確定を延期します。振り返りは `TRADINGAGENTS_OUTPUT_LANGUAGE` に従い、
5 取引間隔の短期的な市場フィードバックだけを表します。中長期の投資仮説が証明または
否定されたとは判断しません。また、`[2026-01-05 → 2026-01-12 | 5d]` のような
