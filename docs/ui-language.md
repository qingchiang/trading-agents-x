...
Calculation entries share one layout whether or not a per-item audit exists. An Incremental diagnostic view reads its direct Full baseline separately and identifies unchanged calculation records by their complete recorded content, not their IDs alone. Baseline checks and Evidence retain their source; they never become a new audit of the update. Missing checks remain unrecorded, and baseline display mismatches remain visible. Baseline lookup failures allow retry without replacing the selected research.

Audit rows default to a compact title, source/status line and recorded-versus-comparison values. Usage, dates, precision, rounded comparisons and technical issue details stay in the existing formula/evidence disclosure. Display mismatch and invalid/missing calculation states remain visible when details are closed; the row does not add a second metadata panel or divider.
