...
def source_0013(tmp_path):
    ...
    with sqlite3.connect(source) as db:
        ...
        for identity, kind, status in [
            ("legacy", None, "succeeded"),
            ("baseline", "full", "succeeded"),
            ("failed-current", "full", "failed"),
            ("cancelled-current", "full", "cancelled"),
        ]:
            db.execute(
                ...
                (
                    identity,
                    status,
                    json.dumps({"ticker": "GOOG", "analysis_date": "2026-09-10"}),
                    '{"historical_missing_connection":true}',
                    "0.4.0",
                    1,
                    False,
                    "{}",
                    ...
                    "2" if kind else None,
                    kind,
                    '{"recorded":true}' if kind else None,
                    None,
                ),
            )
            db.execute(
                ...
                "VALUES (?,1,?,?,0,'{}')",
                (identity, status, f"run:{identity}:attempt:1"),
            )
        db.execute(
            "INSERT INTO research_nodes (run_id,research_kind,created_at) VALUES ('baseline','full','2026-09-10')"
        )
        db.execute(
            "INSERT INTO primary_research_cycles VALUES ('GOOG','baseline','2026-09-10','2026-09-10')"
        )
    return source
...
def test_cutover_preserves_current_failed_runs_and_source_bytes(source_0013, tmp_path):
    ...
    with sqlite3.connect(destination) as db:
        ...
        assert (
            db.execute("SELECT full_run_id FROM primary_research_cycles").fetchone()[0]
            == "baseline"
        )
        assert db.execute("PRAGMA foreign_key_check").fetchall() == []
        assert json.loads(
            db.execute("SELECT audit_snapshot_json FROM runs WHERE id='baseline'").fetchone()[0]
        )["config"] == {"historical_missing_connection": True}
        assert (
            json.loads(
                db.execute("SELECT config_json FROM runs WHERE id='baseline'").fetchone()[0]
            )["deep_binding"]
            is None
        )
...
def test_conversion_retains_connection_identity_secrets_and_reset_template(source_0013, tmp_path):
    ...
    with sqlite3.connect(source_0013) as db:
        db.execute(
            "INSERT INTO model_connections VALUES (?,?,?)", ("retained", definition, "deepseek")
        )
        db.execute(
            "INSERT INTO configuration_credentials VALUES (?,?)",
            ("connection:retained:api_key", "fixture-secret"),
        )
        db.execute(
            "INSERT INTO application_configuration VALUES (1,?,1,8,'2026-09-10')",
            ('{"deep_connection_id":"retained"}',),
        )
    destination = tmp_path / "new.db"
    report = migrate_current(source_0013, destination)
    ...
    with sqlite3.connect(destination) as db:
        assert json.loads(
            db.execute("SELECT definition FROM model_connections").fetchone()[0]
        ) == json.loads(definition)
        assert (
            db.execute("SELECT value FROM configuration_credentials").fetchone()[0]
            == "fixture-secret"
        )
...
def test_cli_conversion_does_not_start_the_application(source_0013, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        main, "_settings", lambda: pytest.fail("offline command must use explicit paths")
    )
    destination = tmp_path / "cli.db"
    result = CliRunner().invoke(
        main.app,
        ["db", "migrate-current", "--source", str(source_0013), "--destination", str(destination)],
    )
...
def test_converted_full_remains_a_baseline_for_offline_incremental(source_0013, tmp_path):
    ...
    from tests.support.incremental import _incremental_service, _pit_collection
    from tradingagents.configuration.settings import AppSettings
    ...
    bundle = EvidenceBundle(
        instrument="GOOG", analysis_date=date(2026, 9, 10), items=(baseline_item,)
    )
    ...
    with sqlite3.connect(source_0013) as db:
        db.execute(
            "UPDATE runs SET information_cutoff_at='2026-09-11T03:59:59.999999' WHERE id='baseline'"
        )
        db.execute(
            "INSERT INTO run_evidence VALUES ('baseline',1,?,?,1,0,'2026-09-11')",
            (bundle.model_dump_json(), bundle.digest),
        )
        db.execute(
            "INSERT INTO decisions (run_id,ticker,market,asset_type,analysis_date,rating,confidence,decision_json,created_at) VALUES ('baseline','GOOG','united_states','stock','2026-09-10',?,?,?,'2026-09-11')",
            (decision.rating.value, decision.confidence.value, decision.model_dump_json()),
        )
        db.execute(
            "INSERT INTO model_connections VALUES (?,?, 'openai')",
            (connection.id, connection.model_dump_json()),
        )
        db.execute(
            "INSERT INTO configuration_credentials VALUES (?, 'offline-fixture-key')",
            (f"connection:{connection.id}:api_key",),
        )
        db.execute(
            "INSERT INTO application_configuration VALUES (1,?,1,1,'2026-09-11')",
            (json.dumps({"llm_provider": "openai", "analysts": ["news"]}),),
        )
    ...
    settings = AppSettings.from_env(
        environ={
            "TRADINGAGENTS_HOME": str(tmp_path),
            "TRADINGAGENTS_DATABASE_PATH": str(destination),
        },
        load_env_files=False,
    )
    repository = RunRepository(settings)
    candidate_item = EvidenceItem.create(
        source="fixture",
        evidence_type="filing",
    ...
    candidate = IncrementalEvidenceCandidate(
        evidence=candidate_item, available_on=date(2026, 9, 11)
    )
...
def test_conversion_normalizes_connection_effort_and_retains_audit(source_0013, tmp_path):
    ...
    with sqlite3.connect(source_0013) as db:
        db.execute(
            "INSERT INTO model_connections VALUES (?,?,?)", ("retained", json.dumps(old), "openai")
        )
        db.execute("UPDATE runs SET config_json=? WHERE id='baseline'", (json.dumps(snapshot),))
    destination = tmp_path / "new.db"
    migrate_current(source_0013, destination)
    with sqlite3.connect(destination) as db:
        converted = ModelConnection.model_validate_json(
            db.execute("SELECT definition FROM model_connections").fetchone()[0]
        )
        assert converted.reasoning_effort == "high"
        assert converted.template["reasoning_effort"] == "medium"
        config, audit = db.execute(
            "SELECT config_json, audit_snapshot_json FROM runs WHERE id='baseline'"
        ).fetchone()
        assert json.loads(config)["deep_binding"]["connection"]["reasoning_effort"] == "high"
        assert json.loads(audit)["config"] == snapshot


def test_cutover_accepts_read_only_wal_source_without_creating_sidecars(source_0013, tmp_path):
    from contextlib import closing
    from shutil import copyfile

    folder = tmp_path / "read-only-source"
    folder.mkdir()
    source = folder / "source.db"
    copyfile(source_0013, source)
    ...
    folder.chmod(0o555)
    try:
        report = migrate_current(source, tmp_path / "converted.db")
        assert report.retained_nodes == 1
        assert source.read_bytes() == before
        assert {path.name for path in folder.iterdir()} == {"source.db"}
    finally:
        folder.chmod(0o755)
        source.chmod(0o644)


def test_cutover_includes_committed_wal_rows_and_rejects_source_changes(
    source_0013, tmp_path, monkeypatch
):
    from contextlib import closing

    from tradingagents.persistence import cutover

    with closing(sqlite3.connect(source_0013)) as writer:
        writer.execute("PRAGMA journal_mode=WAL")
        writer.execute("UPDATE runs SET status='cancelled' WHERE id='failed-current'")
        writer.commit()
        assert Path(str(source_0013) + "-wal").stat().st_size > 0
        destination = tmp_path / "wal-current.db"
        migrate_current(source_0013, destination)
        with closing(sqlite3.connect(destination)) as db:
            assert db.execute("SELECT status FROM runs WHERE id='failed-current'").fetchone() == (
                "cancelled",
            )

        convert = cutover._convert

        def changed_during_conversion(source, target):
            report = convert(source, target)
            writer.execute("UPDATE runs SET status='failed' WHERE id='failed-current'")
            writer.commit()
            return report

        monkeypatch.setattr(cutover, "_convert", changed_during_conversion)
        rejected = tmp_path / "changed.db"
        with pytest.raises(MigrationError, match="Source changed"):
            migrate_current(source_0013, rejected)
