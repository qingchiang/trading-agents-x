...
class PanelDispatchConsistencyTests(unittest.TestCase):
    ...
    def test_every_panel_cell_source_matches_dispatch_ownership(self):
        for spec in self._panel_specs():
            ...
            in_estat, in_boj, in_jp = (
                key in estat.ESTAT_SERIES,
                key in boj.BOJ_SERIES,
                key in jp_macro.JP_SERIES,
            )
            if source == "cn":
            ...
            if source == "estat":
                ...
                self.assertTrue(in_jp, f"{indicator!r} not owned by Japan macro")
                self.assertFalse(in_cn or in_estat or in_boj)
            else:  # fred: a raw FRED id/alias, must be in no JP SERIES dict
                self.assertEqual(source, "fred")
                self.assertFalse(
                    in_cn or in_estat or in_boj or in_jp,
