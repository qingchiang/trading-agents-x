...
class InvestorFlowTests(unittest.TestCase):
    ...
    def test_non_tokyo_ticker_returns_empty(self):
        ...
        with mock.patch.object(js, "fetch_records") as fr:
            self.assertEqual(get_market_investor_flows("AAPL", "2026-06-25").content, "")
            self.assertEqual(get_market_investor_flows("0700.HK", "2026-06-25").content, "")
    ...
    def test_renders_recent_weeks_with_signed_net_flows(self):
        ...
        with _patch(weeks):
            out = get_market_investor_flows("9984.T", "2026-06-25").content
    ...
    def test_caps_to_look_back_weeks(self):
        ...
        with _patch(weeks):
            out = get_market_investor_flows("9984.T", "2026-06-25", look_back_weeks=4).content
    ...
    def test_malformed_curr_date_degrades_without_raising(self):
        ...
        with _patch([_week("2026-06-19", "2026-06-08", "2026-06-12")]):
            out = get_market_investor_flows("9984.T", "not-a-date").content
    ...
    def test_resolved_standard_section_is_used_in_request(self):
        with mock.patch.object(
            ...
            out = get_market_investor_flows("2702.T", "2026-06-25").content
    ...
    def test_unknown_section_does_not_fetch_or_default_to_prime(self):
        with mock.patch.object(
            ...
            out = get_market_investor_flows("9999.T", "2026-06-25").content
...
class MarginBalanceTests(unittest.TestCase):
    def test_non_tokyo_ticker_returns_empty(self):
        with mock.patch.object(js, "fetch_records") as fr:
            self.assertEqual(get_margin_balance("AAPL", "2026-06-25", data_context=request_context()).content, "")
        fr.assert_not_called()
    ...
    def test_renders_recent_weeks_newest_first_with_credit_ratio(self):
        ...
        with _patch(weeks):
            out = get_margin_balance("9984.T", "2026-06-25", data_context=request_context()).content
    ...
    def test_lookahead_guard_excludes_unpublished_week(self):
        ...
        with _patch(weeks):
            out = get_margin_balance("9984.T", "2026-06-22", data_context=request_context()).content
    ...
    def test_year_end_closure_does_not_leak(self):
        ...
        with _patch([_margin("2024-12-27", short="9999999")]):
            self.assertEqual(get_margin_balance("9984.T", "2025-01-03", data_context=request_context()).content, "")
        with _patch([_margin("2024-12-27", short="9999999")]):
            self.assertIn("2024-12-27", get_margin_balance("9984.T", "2025-01-06", data_context=request_context()).content)
    ...
    def test_no_visible_weeks_returns_empty(self):
        ...
        with _patch([_margin("2026-06-24")]):
            self.assertEqual(get_margin_balance("9984.T", "2026-06-25", data_context=request_context()).content, "")

    def test_caps_to_look_back_weeks(self):
        weeks = [_margin(f"2026-0{m}-05") for m in range(1, 6)]  # 5 published weeks
        with _patch(weeks):
            out = get_margin_balance("9984.T", "2026-06-25", look_back_weeks=3, data_context=request_context()).content
    ...
    def test_malformed_curr_date_degrades_to_placeholder(self):
        with _patch([_margin("2026-06-05")]):
            out = get_margin_balance("9984.T", "not-a-date", data_context=request_context()).content
...
class ShortPositionTests(unittest.TestCase):
    def test_non_tokyo_ticker_returns_empty(self):
        with mock.patch.object(js, "fetch_records") as fr:
            self.assertEqual(get_short_positions("AAPL", "2026-06-25", data_context=request_context()).content, "")
        fr.assert_not_called()
    ...
    def test_renders_events_newest_first_with_name_and_ratio(self):
        ...
        with _patch(rows):
            out = get_short_positions("9984.T", "2026-06-25", data_context=request_context()).content
    ...
    def test_unpadded_curr_date_does_not_leak_future(self):
        ...
        with _patch(rows):
            out = get_short_positions("9984.T", "2026-7-5", data_context=request_context()).content
        self.assertIn("2026-06-10", out)
    ...
    def test_no_disclosures_returns_empty(self):
        with _patch([]):
            self.assertEqual(get_short_positions("9984.T", "2026-06-25", data_context=request_context()).content, "")

    def test_caps_to_max_rows(self):
        rows = [_short(f"2026-06-{d:02d}") for d in range(1, 12)]  # 11 in window
        with _patch(rows):
            out = get_short_positions("9984.T", "2026-06-25", max_rows=5, data_context=request_context()).content
    ...
    def test_malformed_curr_date_degrades_to_placeholder(self):
        with _patch([_short("2026-06-10")]):
            out = get_short_positions("9984.T", "not-a-date", data_context=request_context()).content
