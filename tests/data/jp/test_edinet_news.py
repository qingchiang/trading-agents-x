...
from tests.support.data_policy import configure_data, data_config, request_context
...
class NewsRenderTests(unittest.TestCase):
    ...
    def _patch(self, mapping):
        return mock.patch.object(edinet_common, "fetch_documents", side_effect=_by_date(mapping))
    ...
    def test_filters_by_securities_code(self):
        ...
        with self._patch(mapping):
            out = edinet_news.get_news(
                "9984.T", "2026-06-22", "2026-06-22", data_context=request_context()
            )
        self.assertIn("## 9984.T EDINET disclosures", out.content)
        self.assertIn("自社株買い", out.content)
        self.assertNotIn("他社の開示", out.content)
        self.assertIn("EDINET docID: MINE", out.content)
        self.assertNotIn("Effective period:", out.content)
    ...
    def test_window_iterates_each_date_and_sorts_recent_first(self):
        ...
        with self._patch(mapping):
            out = edinet_news.get_news(
                "9984.T", "2026-06-20", "2026-06-22", data_context=request_context()
            )
        self.assertLess(
            out.content.index("新しい開示"), out.content.index("古い開示")
        )  # newest first
    ...
    def test_dates_outside_window_are_never_fetched(self):
        ...
        self.assertIn("in window", out.content)
        self.assertNotIn("future leak", out.content)
    ...
    def test_render_caps_at_news_article_limit(self):
        configure_data({"news_article_limit": 2})
        try:
            mapping = {
                "2026-06-22": [
            ...
            self.assertEqual(out.content.count("### "), 2)
        finally:
    ...
    def test_long_window_is_capped(self):
        ...
        with mock.patch.object(edinet_common, "fetch_documents", mock_fetch):
            out = edinet_news.get_news(
                "9984.T", "2020-01-01", "2026-06-22", data_context=request_context()
            )
        self.assertEqual(mock_fetch.call_count, edinet_common.MAX_WINDOW_CALENDAR_DAYS)
        self.assertIn("between 2026-03-25 and 2026-06-22", out.content)
...
class DocumentCacheTests(unittest.TestCase):
    ...
    def test_settled_date_survives_memory_clear_via_disk(self):
        ...
        with (
            ...
            self.assertEqual(
                edinet_common.documents_on("2026-06-22", data_context=request_context()), records
            )
    ...
    def test_today_is_memory_only_with_short_ttl(self):
        with (
            mock.patch.object(edinet_common, "tokyo_today", return_value=date(2026, 6, 22)),
            mock.patch.object(
            ...
            edinet_common.documents_on("2026-06-22", data_context=request_context())
            edinet_common.documents_on("2026-06-22", data_context=request_context())
            edinet_common.documents_on("2026-06-22", data_context=request_context())

        self.assertEqual(fetch.call_count, 2)
        self.assertFalse(
            Path(edinet_common._disk_file("2026-06-22", data_context=request_context())).exists()
        )

    def test_corrupt_disk_file_degrades_to_fetch_and_replaces_it(self):
        path = Path(edinet_common._disk_file("2026-06-22", data_context=request_context()))
    ...
    def test_disk_cache_prunes_to_file_limit_without_orphan_temps(self):
        with (
            ...
            for date_str in ("2026-06-19", "2026-06-20", "2026-06-21"):
                edinet_common._disk_put(date_str, [], data_context=request_context())

        disk_dir = Path(data_config()["data_cache_dir"]) / "edinet" / "documents" / "v2"
    ...
    def test_memory_cache_is_lru_bounded(self):
        with (
            mock.patch.object(edinet_common, "_MEMORY_MAX_DATES", 2),
            mock.patch.object(edinet_common, "tokyo_today", return_value=date(2026, 6, 23)),
            mock.patch.object(edinet_common, "fetch_documents", return_value=[]),
        ):
            for date_str in ("2026-06-19", "2026-06-20", "2026-06-21"):
                edinet_common.documents_on(date_str, data_context=request_context())
...
class AuthTests(unittest.TestCase):
    def test_missing_key_raises_not_configured(self):
        with use_credentials({}), self.assertRaises(EDINETNotConfiguredError):
    ...
    def test_request_sends_subscription_key_header(self):
        ...
        with (
            use_credentials({"EDINET_API_KEY": "KEY123"}),
    ...
    def test_rate_limit_surfaces_typed_error(self):
        with (
            use_credentials({"EDINET_API_KEY": "K"}),
    ...
    def test_unauthorized_surfaces_not_configured(self):
        with (
            use_credentials({"EDINET_API_KEY": "BAD"}),
...
class RoutingTests(unittest.TestCase):
    ...
    def test_tokyo_ticker_routes_news_to_edinet(self):
        ...
        edn = mock.Mock(return_value=DataResult("EDINET_NEWS"))
        yf = mock.Mock(return_value=DataResult("YF_NEWS"))
        with mock.patch.dict(
            ...
            result = interface.route_to_vendor(
                "get_news", "9984.T", "2026-06-20", "2026-06-22", data_context=request_context()
            )
        self.assertEqual(result.content, "EDINET_NEWS")
