...
class CodeMapTests(unittest.TestCase):
    ...
    def test_seed_resolves_known_tickers(self):
        ...
        self.assertEqual(cm.resolve_edinet_code("9984.T", data_context=request_context()), "E02778")
        self.assertEqual(cm.resolve_edinet_code("7203.T", data_context=request_context()), "E02144")
    ...
    def test_learn_adds_unknown_issuer_and_persists(self):
        self.assertIsNone(cm.resolve_edinet_code("0000.T", data_context=request_context()))
        cm.learn("00000", "E99999", data_context=request_context())  # 5-digit secCode → 4-digit base "0000"
        self.assertEqual(cm.resolve_edinet_code("0000.T", data_context=request_context()), "E99999")
    ...
    def test_learned_cache_survives_reload(self):
        cm.learn("00000", "E99999", data_context=request_context())
    ...
    def test_learn_skips_known_pairs_without_writing(self):
        ...
        cm.learn("99840", "E02778", data_context=request_context())  # SoftBank, already seeded
    ...
    def test_learn_never_overrides_seed(self):
        ...
        cm.learn("99840", "E_WRONG", data_context=request_context())  # SoftBank base 9984 is in the seed
        self.assertEqual(cm.resolve_edinet_code("9984.T", data_context=request_context()), "E02778")
        self.assertFalse(os.path.exists(self._cache_file()))
    ...
    def test_learn_many_batches_into_one_write(self):
        ...
        self.assertEqual(cm.resolve_edinet_code("0000.T", data_context=request_context()), "E90000")
        self.assertEqual(cm.resolve_edinet_code("0001.T", data_context=request_context()), "E90001")

    def test_learn_many_no_changes_skips_write(self):
        with mock.patch.object(cm, "_persist") as persist:
            cm.learn_many([("99840", "E02778"), ("", "E1"), ("123", "E2")], data_context=request_context())
        persist.assert_not_called()

    def test_learn_ignores_incomplete_pairs(self):
        cm.learn("", "E1", data_context=request_context())
        cm.learn("00000", "", data_context=request_context())
        cm.learn(None, None, data_context=request_context())
        self.assertFalse(os.path.exists(self._cache_file()))
    ...
    def test_learn_ignores_non_listed_codes(self):
        ...
        cm.learn("123", "E55555", data_context=request_context())  # too short to be a listing base
        self.assertFalse(os.path.exists(self._cache_file()))
