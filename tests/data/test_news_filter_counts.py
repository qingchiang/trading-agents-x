...
def test_cn_source_counts_survive_shared_memory_cache(source, monkeypatch):
    ...
    def fetch(*_a, **_k):
        ...
        if source == "disclosure":
            ...
            return {
                "announcements": [row("2025-01-01", i) for i in range(98)]
                + [row("2026-09-03", i) for i in (98, 99)]
            }
        ...
        return {
            "data": [row("2025-01-01") for _ in range(98)] + [row("2026-09-03") for _ in range(2)]
        }

    monkeypatch.setattr(news_sources, "_request_json", fetch)
    producer = (
        news_sources.get_disclosure_news
        if source == "disclosure"
        else news_sources.get_research_news
    )
    try:
        ...
        for body in (first, hot):
            assert "upstream_returned=100" in body.content
            assert "date_filtered=98" in body.content
            assert "relevance_filtered=0" in body.content
            assert "duplicates=1" in body.content
            assert "candidates=1" in body.content
        assert len(calls) == 1
    finally:
        news_sources._clear_feed_cache()
...
def test_edinet_counts_cover_security_filter_and_source_cap(monkeypatch):
    ...
    rows = [
        {
            "secCode": "99840",
            "docDescription": "one",
            "submitDateTime": "2026-09-03 10:00",
            "docID": "1",
        },
        {
            "secCode": "99840",
            "docDescription": "two",
            "submitDateTime": "2026-09-03 11:00",
            "docID": "2",
        },
    ...
    with data_policy({**data_config(), "news_article_limit": 1}):
        body = edinet_news.get_news(
            "9984.T", "2026-09-03", "2026-09-03", data_context=request_context()
        )
    assert "upstream_returned=3" in body.content
    assert "relevance_filtered=1" in body.content
    assert "source_truncated=1" in body.content
    assert "candidates=1" in body.content


def test_tdnet_counts_cover_parse_date_security_and_cap(monkeypatch):
    from tests.support.data_policy import data_config, data_policy
    from tests.support.tdnet import _page, _row
    from tradingagents.data.jp import tdnet_news

    page = _page(
        _row(when="2026/09/03 10:00", title="one"),
    ...
    with data_policy({**data_config(), "news_article_limit": 1}):
        body = tdnet_news.get_news(
            "7203.T", "2026-09-03", "2026-09-03", data_context=request_context()
        )
    assert "upstream_returned=5" in body.content
    assert "date_filtered=2" in body.content
    assert "relevance_filtered=1" in body.content
    assert "source_truncated=1" in body.content
    assert "candidates=1" in body.content
