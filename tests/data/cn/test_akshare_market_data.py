...
from tests.support.data_policy import configure_data, request_context
...
def test_tencent_qfq_is_primary_and_records_adjustment(monkeypatch):
    ...
    output = akshare_stock.get_stock(
        "600519.SS", "2026-07-01", "2026-07-19", data_context=request_context()
    )

    assert "# Actual data source: AkShare / Tencent" in output.content
    assert "# Price adjustment: qfq (forward-adjusted)" in output.content
    assert "# Volume unit: shares" in output.content
    assert "# Requested end date: 2026-07-19" in output.content
    assert "# Effective trading date: 2026-07-17" in output.content
    assert "Amount,AmplitudePct,PctChange,PriceChange,TurnoverPct" not in output.content
    em.assert_not_called()
    tencent.assert_called_once_with("sh600519", "2026-07-01", "2026-07-17")
    provenance = list(output.provenance)[0]
    assert provenance.source == "AkShare / Tencent"
    assert "fallback" not in provenance.timing
    cached = akshare_stock.fetch_ohlcv("600519.SS", "2026-07-01", "2026-07-19")
...
def test_eastmoney_cold_fallback_preserves_extended_fields(monkeypatch):
    ...
    output = akshare_stock.get_stock(
        "000001.SZ", "2026-07-01", "2026-07-19", data_context=request_context()
    )
    ...
    assert "Amount,AmplitudePct,PctChange,PriceChange,TurnoverPct" in output.content
    ...
    assert "fallback: Tencent primary retrieval unavailable" in list(output.provenance)[0].timing
    reasons = {issue.reason for issue in provenance_quality_issues(list(output.provenance))}
...
def test_negative_qfq_prices_are_preserved(monkeypatch):
    ...
    result = akshare_stock.fetch_ohlcv("600519.SS", "2026-07-01", "2026-07-19")
...
def test_tencent_equivalent_path_sets_timeout_and_parses_qfq(monkeypatch):
    ...
    frame = akshare_stock._fetch_tencent("sz000001", "2026-07-01", "2026-07-17")

    assert frame.iloc[0]["close"] == "11"
    assert get.call_count == 1
    assert get.call_args.kwargs["timeout"] == common.REQUEST_TIMEOUT
    assert get.call_args.kwargs["params"]["param"] == "sz000001,day,2026-07-01,2026-07-17,640,qfq"
...
def test_tencent_paginates_only_after_a_full_640_row_page(monkeypatch):
    ...
    frame = akshare_stock._fetch_tencent("sh600519", "2024-01-01", "2026-07-17")
...
def test_tencent_short_listing_stops_after_one_page(monkeypatch):
    ...
    frame = akshare_stock._fetch_tencent("sh688981", "2020-01-01", "2026-07-17")
...
def test_tencent_backfill_before_start_is_trimmed_without_another_request(monkeypatch):
    ...
    frame = akshare_stock._fetch_tencent("sh600519", "2026-01-01", "2026-07-17")
...
def test_tencent_rejects_non_qfq_payloads(monkeypatch, wrong_key):
    ...
    with pytest.raises(common.AkShareSchemaError, match="missing qfqday"):
        akshare_stock._fetch_tencent("sz000001", "2026-07-01", "2026-07-17")
...
def test_tencent_page_deduplicates_identical_dates_but_rejects_conflicts(monkeypatch):
    ...
    decoder.decode.return_value = {"data": {"sz000001": {"qfqday": [base, list(base)]}}}
    ...
    conflicting[2] = "99"
    decoder.decode.return_value = {"data": {"sz000001": {"qfqday": [base, conflicting]}}}
    with pytest.raises(common.AkShareSchemaError, match="conflicting duplicate"):
        akshare_stock._fetch_tencent_page("sz000001", "2026-07-01", "2026-07-17", decoder=decoder)
...
def test_both_empty_sources_raise_typed_no_data(monkeypatch):
    ak, _em, _tx = _fake_ak(eastmoney=pd.DataFrame(), tencent=pd.DataFrame())
    monkeypatch.setattr(akshare_stock, "load_akshare", lambda: ak)
    monkeypatch.setattr(akshare_stock, "_fetch_tencent", lambda *_args: pd.DataFrame())
    with pytest.raises(NoMarketDataError, match="returned no qfq rows"):
        akshare_stock.fetch_ohlcv("600519.SS", "2026-07-01", "2026-07-19")
...
def test_successful_frame_is_cached_and_returned_as_copy(monkeypatch):
    ...
    monkeypatch.setattr(akshare_stock, "load_akshare", lambda: ak)
    monkeypatch.setattr(akshare_stock, "_fetch_tencent", tencent)
    first = akshare_stock.fetch_ohlcv("600519.SS", "2026-07-01", "2026-07-19")
    first.frame.loc[0, "Close"] = 999
    second = akshare_stock.fetch_ohlcv("600519.SS", "2026-07-01", "2026-07-19")
...
def test_reversed_date_range_fails_loud():
    with pytest.raises(ValueError, match="start_date.*after end_date"):
        akshare_stock.fetch_ohlcv("600519.SS", "2026-07-20", "2026-07-19")
...
def test_yfinance_fallback_uses_exact_mainland_freshness(monkeypatch):
    ...
    monkeypatch.setattr(calendar, "effective_trade_date", lambda _value: date(2026, 7, 17))
    stale = pd.DataFrame({"Date": [pd.Timestamp("2026-07-16")], "Close": [100.0]})
    with pytest.raises(NoMarketDataError, match="expected mainland trading date"):
        stockstats_utils._assert_ohlcv_not_stale(stale, "2026-07-19", "600519.SS", "600519.SS")


@pytest.mark.unit
def test_yfinance_fallback_excludes_incomplete_mainland_daily_bar(monkeypatch):
    monkeypatch.setattr(calendar, "effective_trade_date", lambda _value: date(2026, 7, 16))
    frame = pd.DataFrame(
        {
            "Open": [100.0, 101.0],
            "High": [102.0, 103.0],
    ...
    output = y_finance.get_YFin_data_online(
        "600519.SS", "2026-07-01", "2026-07-17", data_context=request_context()
    )

    assert "# Effective trading date: 2026-07-16" in output.content
    assert "\n2026-07-16," in output.content
    assert "\n2026-07-17," not in output.content


@pytest.mark.unit
def test_mainland_freshness_rejects_a_row_after_completed_session(monkeypatch):
    monkeypatch.setattr(calendar, "effective_trade_date", lambda _value: date(2026, 7, 16))
    incomplete = pd.DataFrame({"Date": [pd.Timestamp("2026-07-17")], "Close": [100.0]})
    with pytest.raises(NoMarketDataError, match="expected mainland trading date"):
        stockstats_utils._assert_ohlcv_not_stale(incomplete, "2026-07-17", "600519.SS", "600519.SS")
...
def test_yfinance_fallback_fails_closed_when_calendar_is_unavailable(monkeypatch):
    ...
    recent_but_unverified = pd.DataFrame({"Date": [pd.Timestamp("2026-07-16")], "Close": [100.0]})
...
def test_indicator_and_snapshot_reuse_same_qfq_fetch(monkeypatch):
    ...
    indicator = akshare_indicator.get_indicator(
        "600519.SS", "rsi", "2026-07-19", 5, data_context=request_context()
    )
    snapshot = akshare_indicator.get_verified_market_snapshot(
        "600519.SS", "2026-07-19", 30, data_context=request_context()
    )

    assert tencent.call_count == 1
    em.assert_not_called()
    assert "# Actual data source: AkShare / Tencent" in indicator.content
    assert "Effective trading date: 2026-07-17" in indicator.content
    assert "Latest valid indicator observation: 2026-07-17" in indicator.content
    assert "Data source: AkShare / Tencent" in snapshot.content
    assert "Price adjustment: qfq (forward-adjusted)" in snapshot.content
    assert "Latest trading row used: 2026-07-17" in snapshot.content
...
def test_eastmoney_fallback_is_auditable_in_indicator_and_snapshot(monkeypatch):
    ...
    indicator_record = list(indicator.provenance)[0]
    snapshot_record = list(snapshot.provenance)[0]
...
@pytest.mark.unit
def test_endpoint_health_log_records_source_schema_freshness_and_latency(monkeypatch, caplog):
    ak, em, _tx = _fake_ak(eastmoney=_eastmoney_frame())
    monkeypatch.setattr(akshare_stock, "load_akshare", lambda: ak)
    monkeypatch.setattr(
        akshare_stock,
...
def test_fetch_preserves_typed_broken_install_error(monkeypatch):
    ...
    monkeypatch.setattr(akshare_stock, "load_akshare", mock.Mock(side_effect=unavailable))
...
def test_incremental_market_stops_before_eastmoney_after_tencent_rate_limit(
    ...
    with (
        ...
        akshare_stock.fetch_ohlcv("600519.SS", "2026-07-01", "2026-07-17")
...
def test_router_falls_back_to_yfinance_after_akshare_no_data():
    ...
    configure_data({"data_vendors_by_market": {".SS": {"core_stock_apis": "akshare,yfinance"}}})
    ak = mock.Mock(side_effect=NoMarketDataError("600519.SS", "600519.SS", "empty"))
    yf = mock.Mock(return_value=DataResult("YFINANCE_FALLBACK"))
    with mock.patch.dict(
        ...
        output = interface.route_to_vendor(
            "get_stock_data", "600519", "2026-07-01", "2026-07-17", data_context=request_context()
        )
    assert output.content == "YFINANCE_FALLBACK"
    ak.assert_called_once_with(
        "600519.SS", "2026-07-01", "2026-07-17", data_context=request_context()
    )
    yf.assert_called_once_with(
        "600519.SS", "2026-07-01", "2026-07-17", data_context=request_context()
    )
...
def test_router_yfinance_fallback_records_adjustment_provider_change():
    ...
    record = list(output.provenance)[0]
    ...
    assert "auto-adjusted prices" in output.content
...
def test_router_rejects_unsupported_mainland_symbol_before_any_vendor(symbol):
    ak = mock.Mock(return_value=DataResult("AKSHARE"))
    yf = mock.Mock(return_value=DataResult("YFINANCE"))
    with (
        mock.patch.dict(
            interface.VENDOR_METHODS,
            {"get_stock_data": {"akshare": ak, "yfinance": yf}},
        ...
        interface.route_to_vendor(
            "get_stock_data", symbol, "2026-07-01", "2026-07-17", data_context=request_context()
        )
