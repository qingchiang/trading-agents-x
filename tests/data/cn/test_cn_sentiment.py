...
def test_research_signal_prefers_sina_without_querying_eastmoney(monkeypatch):
    ...
    monkeypatch.setattr(
        cn_sentiment,
        "research_rows",
        lambda *_args, data_context: (_ for _ in ()).throw(AssertionError("fallback queried")),
    )
...
def test_holding_api_error_is_not_reported_as_empty(monkeypatch):
    ...
    monkeypatch.setattr(
        cn_sentiment,
        "disclosure_rows",
        lambda *_args, data_context: (_ for _ in ()).throw(AkShareRequestError("CNINFO failed")),
    )

    with pytest.raises(AkShareRequestError, match="bad query"):
        cn_sentiment.get_holding_changes("600519.SS", "2026-01-10", data_context=request_context())
...
def test_sse_margin_rejects_unverifiable_single_row(monkeypatch):
    ...
    with pytest.raises(AkShareSchemaError, match="security-code"):
        cn_sentiment.get_margin_signal("600519.SS", "2026-01-10", data_context=request_context())
...
def test_sz_margin_rejects_matching_row_without_known_metric_columns(monkeypatch):
    ...
    with pytest.raises(AkShareSchemaError, match="recognized financing columns"):
        cn_sentiment.get_margin_signal("000001.SZ", "2026-01-10", data_context=request_context())
