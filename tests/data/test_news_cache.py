...
from tests.support.source_results import news_fixture, source_message
...
def test_news_cache_reuses_refresh_and_retains_disappeared_candidates(tmp_path):
    ...
    first = fetch_news_feed(
        "test", "GOOG", "2026-09-01", "2026-09-05", fetch, now=lambda: current, config=config
    )
    second = fetch_news_feed(
        "test",
        "GOOG",
        "2026-09-01",
        "2026-09-05",
    ...
    third = fetch_news_feed(
        "test",
        "GOOG",
        "2026-09-01",
        "2026-09-05",
        fetch,
        now=lambda: current + timedelta(minutes=16),
        config=config,
    )
    assert len(calls) == 2
    assert all("old event" in body.content for body in (first, second, third))
    assert "2026-09-05T10:00:00+00:00" in third.content
...
def test_cache_refresh_scope_revision_and_failure(tmp_path):
    ...
    def run(start="2026-09-01", budget=100, delta=0, producer=fetch, cfg=None):
        return fetch_news_feed(
            ...
            producer,
            budget=budget,
            now=lambda: current + timedelta(minutes=delta),
    ...
    observations = news_observations(revised.news, "one", "GOOG")
    ...
    assert "refresh failed: RuntimeError" in stale.content and "corrected" in stale.content
    # Another source/configuration cannot read this source's saved material.
    other = fetch_news_feed(
        "two",
        "GOOG",
        "2026-09-01",
        "2026-09-05",
        lambda: news_fixture("No news found"),
        config=config,
    )
    assert "event" not in other.content
    excluded = run(
        cfg={**config, "tool_vendors": {"get_news": "different"}},
        producer=lambda: news_fixture("No news found"),
    )
    assert "event" not in excluded.content
...
def test_cache_eviction_corruption_and_disabled_mode(tmp_path):
    ...
    with sqlite3.connect(path) as connection:
        ...
        assert (
            max(n for (n,) in connection.execute("SELECT count(*) FROM articles GROUP BY scope"))
            <= 2
        )
    late = fetch_news_feed(
        "one",
        "GOOG",
        "2026-09-01",
        "2026-12-10",
        lambda: news_fixture("No news found"),
        config=config,
        now=lambda: current + timedelta(days=96),
    )
    assert not late.news
    ...
    assert (
        fetch_news_feed(
            ...
            "2026-09-05",
            fetch,
            config={**config, "news_cache_enabled": False},
        )
        == body
    )
    assert len(calls) == 2
...
def test_cache_concurrent_writes_remain_readable(tmp_path):
    ...
    assert all("event" in result.content for result in results)
    merged = fetch_news_feed(
        "one",
        "GOOG",
        "2026-09-01",
        "2026-09-05",
    ...
    assert all(f"event{i}" in merged.content for i in range(4))
...
def test_cached_material_enters_full_and_incremental_with_original_time(tmp_path):
    ...
    fetch_news_feed(
        *args,
        lambda: news_fixture("## news\n\n### event\nPublished: 2026-09-03T10:00:00Z"),
        config=config,
        now=lambda: current,
    )

    def route(*_args, **_kwargs):
        block = fetch_news_feed(
            *args,
            lambda: news_fixture("No news found"),
            config=config,
            now=lambda: current + timedelta(minutes=16),
    ...
    full = collect_evidence(
        [source_message(body, tool_call_id="news", name="get_news")],
        requested_date=current.date(),
        analyst="news",
    )
    request = _request(
        enabled_domains=("news",),
        baseline=current.date() - timedelta(days=4),
        target=current.date(),
    ...
    assert (
        full[0].provenance["observation_identity"]
        == incremental[0].evidence.provenance["observation_identity"]
    )
...
def test_refresh_failure_survives_incremental_admission_without_changing_article_identity(
    ...
    def fetch():
        if attempt_time == current:
            return news_fixture("## news\n\n### event\nPublished: 2026-09-03T10:00:00Z")
        raise RuntimeError("source unavailable")

    def route(method, *_args, **_kwargs):
        if method != "get_news":
            return news_fixture("No news found")
        block = fetch_news_feed(
            "yfinance",
            "GOOG",
            "2026-09-01",
    ...
    original = collect_us_incremental(
        request, route_to_vendor=route, now=lambda: current, data_context=request_context()
    )
    attempt_time = current + timedelta(minutes=16)
    refreshed = collect_us_incremental(
        request, route_to_vendor=route, now=lambda: attempt_time, data_context=request_context()
    )
    ...
    assert (
        items[0].provenance["observation_identity"]
        == original.evidence[0].evidence.provenance["observation_identity"]
    )
...
def test_cache_tracks_content_reversion_as_a_new_observed_version(tmp_path):
    ...
    past = fetch_news_feed(
        ...
        lambda: news_fixture("No news found"),
        config=config,
        now=lambda: current + timedelta(days=2, minutes=1),
    )
    assert past.news[0].retrieved_at == first.news[0].retrieved_at


def test_full_news_separates_availability_diagnostics_from_articles():

    from tradingagents.data.news_selection import news_observations, news_result
    from tradingagents.domain.data import ProvenanceRecord
    from tradingagents.domain.news import NewsCandidate
    from tradingagents.research.full.evidence import collect_evidence

    current = datetime(2026, 9, 5, 10, tzinfo=UTC)
    article = NewsCandidate(
        "cninfo",
        "real event",
        "### real event",
    ...
    success = ProvenanceRecord(
        "get_news", "cninfo", "2026-09-01 to 2026-09-05", "2026-09-04", "publication-date filtered"
    )
    failure = ProvenanceRecord(
        "get_news", "eastmoney", "2026-09-01 to 2026-09-05", "unknown", "source unavailable"
    )
    body = news_result("## feed", [article]).with_provenance(success).with_scope("point_in_time")
    from dataclasses import replace

    body = replace(body, observations=news_observations([article], "cninfo", "600519.SS"))
    body = DataResult.combine(
        (
            body,
            DataResult("### Source availability notes\n<Eastmoney unavailable>").with_provenance(
                failure
            ),
        )
    )
    ...
    assert any(
        origin.source == "eastmoney" and "unavailable" in origin.timing
...
def test_cn_query_recovery_does_not_revise_unchanged_cached_article(tmp_path, monkeypatch):
    ...
    partial = True
    monkeypatch.setattr(
        google_news, "_company_names", lambda _: ("贵州茅台酒股份有限公司", "贵州茅台")
    )
        ...
        return [
            {
                "title": "贵州茅台发布业绩",
                "source": "证券时报",
                "published": datetime(2026, 9, 4, 10),
            }
        ]
    ...
    args = (
        ...
        "2026-09-01",
        "2026-09-05",
        lambda: google_news.get_news(
            "600519.SS", "2026-09-01", "2026-09-05", data_context=request_context()
        ),
    )
    ...
    first_header, first_rows = first.news_header, first.news
    recovered_header, recovered_rows = recovered.news_header, recovered.news
