...
def test_china_borrows_unused_source_quota_without_another_fetch():
    from tradingagents.data.news_selection import merge_news_blocks

    blocks = [news_fixture("## official\n\n### filing\nDisclosed: 2026-09-01")]
    blocks.append(
        news_fixture(
            "## research\n\n"
    ...
    assert sum(block.content.count("### ") for block in kept) == 30
...
def test_company_selection_keeps_latest_intraday_items_from_cache_order():
    ...
    block = news_fixture(
        "## news\n\n"
        + "\n\n".join(
            f"### event {hour}\nPublished: 2026-09-05T{hour}:00:00Z" for hour in ("08", "09", "10")
        )
    )
    selected, counts = merge_news_blocks([block], 2, "2026-09-01", "2026-09-05")
    assert [row.title for row in selected[0].news] == ["event 10", "event 09"]
