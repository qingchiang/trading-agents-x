...
class MarketRoutingTests(unittest.TestCase):
    ...
    def test_jp_suffix_routes_to_market_vendor(self):
        ...
        self.assertEqual(result.content, "JP")
    ...
    def test_us_ticker_unaffected_by_market_routes(self):
        ...
        self.assertEqual(result.content, "YF")
    ...
    def test_bare_a_share_is_normalized_before_market_routing(self):
        ...
        self.assertEqual(result.content, "CN")
    ...
    def test_verified_snapshot_uses_jp_technical_vendor(self):
        ...
        self.assertEqual(result.content, "JQ SNAPSHOT")
    ...
    def test_verified_snapshot_us_uses_default_yfinance(self):
        ...
        self.assertEqual(result.content, "YF SNAPSHOT")
    ...
    def test_verified_snapshot_falls_back_when_jquants_unconfigured(self):
        ...
        self.assertEqual(result.content, "YF FALLBACK")
    ...
    def test_market_without_category_falls_back_to_default(self):
        ...
        configure_data({"data_vendors_by_market": {".T": {"core_stock_apis": "jquants"}}})
        with self._route(
            "get_fundamentals", {"yfinance": _returns("YF_F"), "jquants": _returns("JP_F")}
        ):
            result = interface.route_to_vendor(
                "get_fundamentals", "9984.T", "2026-01-01", data_context=request_context()
            )
        self.assertEqual(result.content, "YF_F")
    ...
    def test_tool_vendor_overrides_market_route(self):
        ...
        with self._route(
            "get_stock_data", {"jquants": _returns("JP"), "alpha_vantage": _returns("AV")}
        ):
            result = interface.route_to_vendor(
                "get_stock_data",
                "9984.T",
        ...
        self.assertEqual(result.content, "AV")
    ...
    def test_macro_is_market_agnostic_even_if_routed(self):
        ...
        with self._route("get_macro_indicators", {"fred": _returns("FRED"), "boj": boj}):
            result = interface.route_to_vendor(
                "get_macro_indicators", "cpi", "2026-01-01", data_context=request_context()
            )
        self.assertEqual(result.content, "FRED")
    ...
    def test_global_news_stays_global_when_ticker_news_is_routed(self):
        ...
        with self._route("get_global_news", {"yfinance": _returns("GLOBAL"), "edinet_news": jp}):
            result = interface.route_to_vendor(
                "get_global_news", "2026-01-01", 7, 10, data_context=request_context()
            )
    ...
    def test_ticker_news_is_routed_while_global_is_not(self):
        ...
        configure_data({"data_vendors_by_market": {".T": {"news_data": "edinet_news"}}})
        with self._route(
            "get_news", {"yfinance": _returns("YF_NEWS"), "edinet_news": _returns("JP_NEWS")}
        ):
            result = interface.route_to_vendor(
                "get_news", "9984.T", "2026-01-01", "2026-01-10", data_context=request_context()
            )
        self.assertEqual(result.content, "JP_NEWS")
    ...
    def test_default_config_routes_dot_t_to_japanese_vendors(self):
        ...
        self.assertEqual(
            routes["fundamental_data"], "jp_fundamentals,jp_statements,jquants,yfinance"
        )
    ...
    def test_china_fundamental_chain_selects_method_specific_assembler(self):
        ...
        with (
            ...
            fundamentals = interface.route_to_vendor(
                "get_fundamentals", "600519.SS", "2026-04-01", data_context=request_context()
            )
        ...
        self.assertEqual(fundamentals.content, "CN FUNDAMENTALS")
        self.assertEqual(statement.content, "CN STATEMENT")
    ...
    def test_china_news_route_falls_back_only_after_cn_no_data(self):
        cn_vendor = mock.Mock(side_effect=NoMarketDataError("600519.SS", detail="empty CN window"))
        yfinance = mock.Mock(side_effect=_returns("YF NEWS"))
        with self._route("get_news", {"cn_news": cn_vendor, "yfinance": yfinance}):
            output = interface.route_to_vendor(
                "get_news", "600519.SS", "2026-01-01", "2026-01-10", data_context=request_context()
            )

        self.assertEqual(output.content, "YF NEWS")
    ...
    def test_cn_market_route_does_not_change_global_news_vendor(self):
        ...
        with self._route("get_global_news", {"yfinance": _returns("GLOBAL NEWS")}):
            output = interface.route_to_vendor(
                "get_global_news", "2026-01-10", data_context=request_context()
            )

        self.assertEqual(output.content, "GLOBAL NEWS")
    ...
    def test_china_fundamental_route_rejects_invalid_date_before_vendor(self):
        ...
        with (
            ...
            interface.route_to_vendor(
                "get_fundamentals", "600519.SS", "bad-date", data_context=request_context()
            )
    ...
    def test_china_statement_fallback_keeps_primary_availability_note(self):
        ...
        cn_statements = mock.Mock(
            side_effect=NoMarketDataError(
                ...
                availability_notes=(DataResult(note),),
            )
        )
        with self._route(
            "get_income_statement",
        ...
        self.assertIn("YF STATEMENT", output.content)
        self.assertIn("### Source availability notes", output.content)
        self.assertIn(note, output.content)
    ...
    def test_market_route_validation_rejects_unknown_vendor(self):
        config = copy.deepcopy(default_config.DEFAULT_CONFIG)
        config["data_vendors_by_market"][".SS"]["core_stock_apis"] = "bogus_vendor,yfinance"
    ...
    def test_market_route_validation_rejects_duplicate_vendor(self):
        config = copy.deepcopy(default_config.DEFAULT_CONFIG)
        config["data_vendors_by_market"][".SS"]["core_stock_apis"] = "yfinance,yfinance"
