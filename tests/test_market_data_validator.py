...
class TestVerifiedSnapshot:
    def test_excludes_future_rows(self, monkeypatch):
        ...
        snap = validator.build_verified_market_snapshot("COF", "2026-05-13", data_context=request_context())
    ...
    def test_uses_previous_trading_day_when_date_is_weekend(self, monkeypatch):
        ...
        snap = validator.build_verified_market_snapshot("COF", "2026-05-16", data_context=request_context())
    ...
    def test_raises_when_no_rows_on_or_before_date(self, monkeypatch):
        ...
        with pytest.raises(ValueError):
            validator.build_verified_market_snapshot("COF", "2020-01-01", data_context=request_context())
    ...
    def test_raises_on_empty_data(self, monkeypatch):
        ...
        with pytest.raises(ValueError):
            validator.build_verified_market_snapshot("COF", "2026-05-13", data_context=request_context())

    def test_look_back_window_capped_at_30(self, monkeypatch):
        monkeypatch.setattr(validator, "load_ohlcv", lambda s, d, *, data_context: _sample_ohlcv())
        snap = validator.build_verified_market_snapshot("COF", "2026-05-20", look_back_days=999, data_context=request_context())
