...
def test_stock_window_uses_new_york_calendar_not_utc_date():
    ...
    assert ynews._in_news_window(late_friday_in_new_york, start, end, ticker="NVDA")
    assert not ynews._in_news_window(saturday_in_new_york, start, end, ticker="NVDA")
...
def test_global_windows_use_utc_calendar():
    ...
    assert ynews._in_news_window(first_hour_of_tokyo_date, start, end, ticker="9984.T")
...
def test_hong_kong_window_uses_local_calendar_not_new_york():
    ...
    first_hour_of_next_hong_kong_date = datetime(2025, 5, 9, 16, 30, tzinfo=UTC)
...
def test_global_news_future_flat_article_excluded(monkeypatch):
    ...
    out = ynews.get_global_news_yfinance(
        "2025-05-09", look_back_days=7, limit=10, data_context=request_context()
    )
    assert "PAST EVENT" in out.content
    assert "FUTURE EVENT" not in out.content  # #1007
...
def test_global_news_empty_after_filter_is_informative(monkeypatch):
    ...
    assert "No global news found" in out.content
    assert "upstream_returned=" in out.content and "date_filtered=" in out.content
    assert "###" not in out.content  # no empty article body
...
def test_global_news_continues_past_expired_candidates_and_preserves_publication(monkeypatch):
            ...
            self.news = [
                {
                    ...
                    "providerPublishTime": _epoch(
                        "2025-01-01" if Search.calls == 1 else "2025-05-05"
                    ),
                }
            ]
    ...
    output = ynews.get_global_news_yfinance(
        "2025-05-09", look_back_days=7, limit=1, data_context=request_context()
    )
    assert "eligible" in output.content
    assert "Published: 2025-05-05T00:00:00+00:00" in output.content
...
def test_global_news_continues_until_canonical_candidates_reach_budget(monkeypatch):
    ...
    second_query = [article(f"SECOND QUERY EVENT {index}", index + 10) for index in range(10)]
    queries = []
    ...
    assert "candidates=15" in output.content
    assert "truncated=5" in output.content
    assert "SECOND QUERY EVENT 4" in output.content
    assert "SECOND QUERY EVENT 5" not in output.content


@pytest.mark.parametrize("limit,expected_queries", [(1, 1), (20, 2)])
def test_global_output_target_is_independent_of_per_query_candidates(
    monkeypatch, limit, expected_queries
):
    from tests.data_policy import data_config, data_policy
    ...
    class Search:
        def __init__(self, *, query, news_count, **kwargs):
            ...
            self.news = [
                {"title": f"{query} event {i}", "providerPublishTime": _epoch("2025-05-05")}
    ...
    with data_policy(
        ...
        output = ynews.get_global_news_yfinance(
            "2025-05-09", limit=limit, data_context=request_context()
        )
...
def test_global_larger_output_target_cannot_reuse_smaller_refresh(tmp_path, monkeypatch):
    ...
    with data_policy(
        ...
        output = ynews.get_global_news_yfinance(
            "2025-05-09", limit=20, data_context=request_context()
        )
