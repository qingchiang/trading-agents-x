...
class JPFundamentalsTests(unittest.TestCase):
    def setUp(self):
        ...
        patcher = mock.patch.object(
            jp_fundamentals, "get_analyst_forward", return_value=(None, None)
        )
    ...
    def _run(self, records, price_df, bench_df=None):
        ...
        with (
            mock.patch.object(
                jp_fundamentals.jqf, "get_fundamentals", return_value=DataResult("BASE-OVERVIEW")
            ),
            mock.patch.object(
                jp_fundamentals.jqf, "fetch_periods", return_value=("7011.T", records)
            ),
            ...
            return jp_fundamentals.get_fundamentals(
                "7011.T", "2026-06-26", data_context=request_context()
            )
    ...
    def test_fy_case_matches_golden_ratios(self):
        ...
        self.assertIn("BASE-OVERVIEW", out.content)  # official summary preserved
        self.assertIn("Market cap: ¥11.99T", out.content)  # 3567 × (ShOutFY−TrShFY)
        self.assertIn("shares 3,360,209,340", out.content)
        self.assertIn("PE: 36.08 (TTM)", out.content)  # 3567/98.86
        self.assertIn("PB: 3.88", out.content)  # 3567/919.16
        self.assertIn("Dividend yield: 0.70% (DivAnn 25.00)", out.content)
        self.assertIn("Payout: 25.30%", out.content)
        self.assertIn("Forward PE: 31.54 (company guidance / 会社予想, EPS 113.09)", out.content)
        self.assertIn("PEG: 2.19", out.content)
        self.assertIn("1yr growth +14.4%", out.content)
        self.assertIn("Net margin: 6.68% (TTM)", out.content)
        self.assertIn("ROE: 10.76%", out.content)  # EPS/BPS (owners' basis), consistent with PB
        self.assertIn("ROA: 4.02%", out.content)
        self.assertIn("Equity ratio: 0.37", out.content)
        self.assertIn("52-week range: 3171.00 – 5208.00", out.content)
        self.assertIn("Beta (vs TOPIX, 3yr weekly): N/A", out.content)  # 2-row frame < min overlap
    ...
    def test_beta_regresses_weekly_returns_against_topix(self):
        ...
        self.assertIn("Beta (vs TOPIX, 3yr weekly): 1.30", out.content)
    ...
    def test_beta_na_when_benchmark_unavailable(self):
        ...
        with (
            ...
            mock.patch.object(
                jp_fundamentals, "fetch_topix_closes", side_effect=RuntimeError("no index")
            ),
        ):
            out = jp_fundamentals.get_fundamentals(
                "7011.T", "2026-06-26", data_context=request_context()
            )
        self.assertIn("Beta (vs TOPIX, 3yr weekly): N/A", out.content)
        self.assertIn("PE: 36.08 (TTM)", out.content)  # rest of the block intact
    ...
    def test_transient_topix_error_only_drops_beta(self):
        ...
        with (
            ...
            mock.patch.object(
                jp_fundamentals, "fetch_topix_closes", side_effect=requests.ConnectionError("reset")
            ),
        ):
            out = jp_fundamentals.get_fundamentals(
                "7011.T", "2026-06-26", data_context=request_context()
            )
        self.assertIn("BASE-OVERVIEW", out.content)
        self.assertIn("PE: 36.08 (TTM)", out.content)
        self.assertIn("Beta (vs TOPIX, 3yr weekly): N/A", out.content)
        self.assertNotIn("ratio computation failed", out.content)
    ...
    def _run_live(self, analyst, curr_date, records=None):
        ...
        with (
            ...
            mock.patch.object(
                jp_fundamentals.jqf, "fetch_periods", return_value=("7011.T", records or [_FY])
            ),
            mock.patch.object(
                jp_fundamentals,
                "_fetch_ohlcv_frame",
            ...
            out = jp_fundamentals.get_fundamentals(
                "7011.T", curr_date, data_context=request_context()
            )
        return out, gaf
    ...
    def test_live_mode_adds_analyst_forward_overlay(self):
        ...
        self.assertIn(
            "company guidance / 会社予想, EPS 113.09", out.content
        )  # date-safe, still there
        self.assertIn("Forward PE: 38.49 (analyst consensus, live only; requested", out.content)
        self.assertIn("16 analysts, not point-in-time historical data, EPS 92.67", out.content)
        self.assertIn("company guidance +14.4% vs analyst -6.3% (divergent)", out.content)
        self.assertIn("| forward_eps | 92.67 | currency | JPY/share |", out.content)
        self.assertIn("| forward_pe |", out.content)
        self.assertIn("| forward_eps_growth |", out.content)
    ...
    def test_flat_company_guidance_vs_decline_reads_divergent(self):
        ...
        self.assertIn("company guidance +0.0% vs analyst -9.0% (divergent)", out.content)
    ...
    def test_backtest_mode_hides_analyst_forward(self):
        ...
        self.assertIn("company guidance / 会社予想", out.content)  # date-safe forward stays
        self.assertNotIn("analyst consensus, live only", out.content)
    ...
    def test_live_mode_omits_overlay_when_analyst_data_absent(self):
        ...
        self.assertIn("company guidance / 会社予想", out.content)
        self.assertNotIn("analyst consensus, live only", out.content)
    ...
    def test_ttm_rolls_cumulative_quarters(self):
        ...
        self.assertIn("PE: 43.05 (TTM)", out.content)  # 3567 / 82.85
        self.assertNotIn("TTM unavailable", out.content)  # rolling succeeded
    ...
    def test_ttm_degrades_to_fy_when_rolling_inputs_missing(self):
        ...
        self.assertIn("PE: 48.84 (FY (TTM unavailable))", out.content)  # 3567 / 73.04
    ...
    def test_price_fetch_failure_degrades_price_ratios_but_keeps_summary(self):
        ...
        with (
            ...
            mock.patch.object(jp_fundamentals.jqf, "fetch_periods", return_value=("7011.T", [_FY])),
            mock.patch.object(
                jp_fundamentals, "_fetch_ohlcv_frame", side_effect=RuntimeError("halted")
            ),
        ):
            out = jp_fundamentals.get_fundamentals(
                "7011.T", "2026-06-26", data_context=request_context()
        ...
        self.assertIn("BASE-OVERVIEW", out.content)
        self.assertIn("Price: N/A", out.content)
        self.assertIn("PE: N/A", out.content)
        self.assertIn("Market cap: N/A", out.content)
        self.assertIn("Net margin: 6.68% (TTM)", out.content)  # price-free ratio still computed
        self.assertIn("ROE: 10.76%", out.content)
        self.assertIn("Beta (vs TOPIX, 3yr weekly): N/A", out.content)  # no price frame → no beta
        sources = {record.source for record in list(out.provenance)}
    ...
    def test_price_provenance_is_only_emitted_for_an_effective_price(self):
        out = self._run([_FY], _price_df(3567.0, 5208.0, 3171.0))
        records = {record.source: record for record in list(out.provenance)}
        self.assertEqual(
    ...
    def test_look_ahead_date_is_propagated_to_both_sources(self):
        ...
        with (
            mock.patch.object(
                jp_fundamentals.jqf, "get_fundamentals", return_value=DataResult("BASE")
            ),
    ...
    def test_forecast_decline_suppresses_negative_peg(self):
        ...
        self.assertIn("PEG: N/A", out.content)
        self.assertIn("1yr growth -9.0%", out.content)
        self.assertNotIn("PEG: -", out.content)
    ...
    def test_no_statements_notes_unavailable_but_keeps_summary(self):
        ...
        self.assertIn("BASE-OVERVIEW", out.content)
        self.assertIn("no statement disclosures", out.content)
