...
def _context(
    ...
    return RunContext(
        ...
        instrument_context=f"The instrument is {ticker}.",
...
@pytest.mark.parametrize("ticker", ["NVDA", "7203.T", "600519.SS"])
def test_profiles_share_contract_but_use_distinct_topologies(
    ...
    execution = graph.execute(
        _context(app_settings, profile, artifact_writer=artifacts.append, ticker=ticker),
