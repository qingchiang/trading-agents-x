...
def test_reflector_normalizes_nested_and_provider_native_usage() -> None:
    class _NestedUsageLLM(_RecordingLLM):
        def invoke(self, messages):
            response = super().invoke(messages)
    ...
    draft = OutcomeReflector(_NestedUsageLLM()).reflect(
        decision="Fixture decision",
        raw_return=0.01,
        alpha_return=-0.02,
        benchmark="SPY",
    ...
    class _ProviderUsageLLM(_RecordingLLM):
        def invoke(self, messages):
            response = super().invoke(messages)
    ...
    provider_draft = OutcomeReflector(_ProviderUsageLLM()).reflect(
        decision="Fixture decision",
        raw_return=0.01,
        alpha_return=-0.02,
        benchmark="SPY",
    ...
    assert provider_draft.usage == {
        "input_tokens": 11,
        "output_tokens": 5,
        "cache_hit_input_tokens": 4,
        "cache_miss_input_tokens": 7,
        "reasoning_output_tokens": 2,
        "provider_reported_cost_usd": 0.012,
    }


def test_invalid_candidate_audit_preserves_original_digest_and_length() -> None:
    raw_candidate = '{"method_lesson":"' + ("x" * 5_000) + '","api_key":"secret"}'
    reflector = OutcomeReflector(_RecordingLLM(raw_candidate))
    ...
    assert len(error.value.candidate) <= 4_000
    assert "secret" not in error.value.candidate
    assert error.value.candidate_digest == sha256(raw_candidate.encode()).hexdigest()
    assert error.value.candidate_length == len(raw_candidate)
    assert any(issue.startswith("string_too_long:") for issue in error.value.validation_issues)
