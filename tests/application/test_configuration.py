...
def test_configuration_save_is_atomic_and_credentials_are_only_explicitly_revealed(tmp_path):
    ...
    saved = store.save(
        ConfigurationPatch(
            ...
            credentials={"FRED_API_KEY": "test-private-value"},
        )
    )
    assert saved.values.output_language == "ja"
    assert saved.credentials["FRED_API_KEY"]
    assert "test-private-value" not in saved.model_dump_json()
    assert store.reveal("FRED_API_KEY") == "test-private-value"
...
def test_import_preview_is_redacted_and_imported_values_do_not_follow_environment(tmp_path):
    settings = AppSettings.from_env(
        environ={
            ...
            "FRED_API_KEY": "import-private-key",
            "TRADINGAGENTS_LLM_BACKEND_URL": "https://relay.example/v1",
        }
    )
    upgrade_database(settings)
    store = ConfigurationStore(settings)
    assert store.read().values.models.deep.connection_id == "default"
    ...
    assert resolved.deep_binding.connection.transport.base_url == "https://relay.example/v1"
    assert request.output_language == "ja"
    store.save(ConfigurationPatch(revision=1, credentials={"FRED_API_KEY": None}))
    assert store.reveal("FRED_API_KEY") is None
    ...
    with pytest.raises(ValueError):
        store.save(
            ConfigurationPatch(
                ...
                credentials={"FRED_API_KEY": "bad-patch-key"},
            )
        )
...
def test_import_precedence_invalid_fields_and_stale_source(tmp_path):
    ...
    assert store.reveal_connection("default", "api_key") == "process-value"
