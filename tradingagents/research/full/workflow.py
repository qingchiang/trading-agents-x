...
from tradingagents.research.synthesis.evidence_context import build_analyst_evidence_context
...
class ResearchGraph:
    ...
    def _attach_research_workflow(
        ...
        workflow.add_node(
            "case.bull",
            self._create_case_node(PERSPECTIVE_SPECS["bull"]),
        )
        workflow.add_node(
            "case.bear",
            self._create_case_node(PERSPECTIVE_SPECS["bear"]),
        )
        ...
        workflow.add_node(
            "rebuttal.bull",
            self._create_rebuttal_node(PERSPECTIVE_SPECS["bull"]),
        )
        workflow.add_node(
            "rebuttal.bear",
            self._create_rebuttal_node(PERSPECTIVE_SPECS["bear"]),
        )
        ...
        if self.profile is RunProfile.STANDARD:
            workflow.add_node(
                "risk.review",
                self._create_risk_node(PERSPECTIVE_SPECS["risk"]),
            )
            ...
            for key in ("aggressive", "neutral", "conservative"):
                ...
                workflow.add_node(
                    name,
                    self._create_risk_node(PERSPECTIVE_SPECS[key]),
                )
    ...
    def _create_debate_agenda_node(self):
        def agenda_node(
            ...
            self._write_artifact(
                ...
                prompt_version="debate-agenda-v10-focused-context",
            )
            self._finish_node(
                runtime,
                node,
