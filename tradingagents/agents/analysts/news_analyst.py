...
def create_news_analyst(llm):
    def news_analyst_node(state):
        ...
        system_message = (
            ...
            " Preserve each material news item's supplied publisher and publication/disclosure date, and preserve source/observation dates for exact macro claims. Do not create a data-provenance appendix yourself; the workflow may append one in audit mode."
        ...
        if len(result.tool_calls) == 0:
            explicit_records = extract_provenance(macro_panel)
            ...
            report = append_provenance_appendix(
                result.content,
                [*extract_provenance(state["messages"]), *explicit_records],
                expected=(("get_news", "routed ticker news"),),
