...
def _technical_frame(symbol: str, curr_date: str):
    start = (datetime.strptime(curr_date, "%Y-%m-%d") - relativedelta(days=_WARMUP_DAYS)).strftime(
        "%Y-%m-%d"
    )
    return fetch_ohlcv(symbol, start, curr_date)


@source_metadata("get_indicators", "akshare")
def get_indicator(
    symbol: str,
    indicator: str,
    curr_date: str,
    ...
    *,
    ...
    return DataResult(
        metadata + render_indicator_window(result.frame, indicator, curr_date, look_back_days)
    ).with_provenance(
        ProvenanceRecord(
...
@source_metadata("get_verified_market_snapshot", "akshare")
