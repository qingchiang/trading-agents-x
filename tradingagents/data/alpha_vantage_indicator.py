...
@source_metadata("get_indicators", "alpha_vantage")
def get_indicator(
    symbol: str,
    indicator: str,
    curr_date: str,
    ...
    *,
    ...
    try:
        ...
        if indicator == "close_50_sma":
            ...
            return DataResult(f"Error: Indicator {indicator} not implemented yet.")

        # Parse CSV data and extract values for the date range
        lines = data.strip().split("\n")
        if len(lines) < 2:
            return DataResult(f"Error: No data returned for {indicator}")
        ...
        return DataResult(f"Error retrieving {indicator} data: {str(e)}")
