    ...
    if isinstance(value, str):
        try:
            value = datetime.fromisoformat(
                re.sub(r"\s+(?:JST|CST)$", "", value).replace("Z", "+00:00")
            )
        except ValueError:
...
def select_temporal[T](
    ...
    *,
    ...
    recent = [pair for pair in ordered if (day(pair[1]) or date.min) >= recent_start]
    older = [pair for pair in ordered if start <= (day(pair[1]) or date.min) < recent_start]
...
def merge_news_blocks(blocks, limit, start_date=None, end_date=None, quotas=None):
    ...
    returned = []
    for block in blocks:
        header, candidates = block.news_header or block.content, block.news
...
def news_result(
    header: str,
    rows: list[NewsCandidate] | tuple[NewsCandidate, ...],
    *,
    diagnostics: tuple[DataDiagnostic, ...] = (),
) -> DataResult[str]:
    return DataResult(
        header + ("\n\n" + "\n\n".join(render_candidate(row) for row in rows) if rows else ""),
        news=tuple(rows),
        news_header=header,
...
def news_observations(rows, source: str, ticker: str, *, global_news=False):
    ...
    for row in rows:
        ...
        try:
            ...
            stamp = datetime.fromisoformat(
                re.sub(r"\s+(?:JST|CST)$", "", row.published).replace("Z", "+00:00")
            )
            ...
            pass
        timing = (
            "near-live revision; revision publication unknown"
            if row.revision
            else "publication dated; bounded news selection"
            if row.day
            else "near-live undated news; publication time unknown"
        )
        if row.refresh_failure:
            timing += (
                f"; news cache refresh failed: {row.refresh_failure}; retained cached material"
            )
        observations.append(
            make_observation(
                row.source or source,
                "global_news_article" if global_news else "news_article",
                row.record_id or row.link or row.title,
                {
                    "title": row.title,
                    "content": row.content,
                    "link": row.link,
                    "original_publication": row.published,
                    "revision": row.revision,
                },
                effective_date=row.effective_date or row.day,
...
def finalize_news(block, source, ticker, start, end, limit, *, global_news=False):
    header, rows = block.news_header or block.content, block.news
    if not rows:
        return block
    merged, counts = merge_news_blocks([block], limit, start, end)
    selected = merged[0] if merged else replace(block, content=header, news=())
    result = selected.content
    count = counts[0]
    tiers = {
        tier: len(re.findall(r"(?m)^### \[" + tier + r"\]", result))
        for tier in ("direct", "candidate", "context")
    }
    result = re.sub(
        r"kept=\d+ \(direct=\d+, candidate=\d+, context=\d+\)",
        f"kept={count.kept} (direct={tiers['direct']}, candidate={tiers['candidate']}, context={tiers['context']})",
        result,
    )
    ...
    return replace(
        selected,
        content=result,
        observations=news_observations(selected.news, source, ticker, global_news=global_news),
    )
