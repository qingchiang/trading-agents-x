...
def _collect_market(request, routed, now):
    try:
        ...
        current = tuple(point for point in series.points if request.window_start < point.completed_at <= request.window_end)
        if not current:
            return (
                CollectionDomainResult(
                    domain="market", state=CollectionResultState.EMPTY, sources=(source,),
        ...
        item = EvidenceItem.create(
            source=source.source, evidence_type="adjusted_close",
            requested_date=request.analysis_cutoff, effective_date=point.session,
            available_at=point.completed_at, value=point.adjusted_close, unit="currency",
        ...
        return (
            CollectionDomainResult(
                domain="market", state=(CollectionResultState.PARTIAL if omitted else CollectionResultState.DATA),
                sources=(source,), observed_from=series.points[0].completed_at,
                observed_through=series.points[-1].completed_at,
                temporal_bases=(CollectionTemporalBasis.PIT,), evidence_refs=(item.ref,),
                diagnostic=(CollectionDiagnostic(code="inadmissible_market_rows_omitted") if omitted else None),
            ),
            IncrementalEvidenceCandidate(evidence=item),
            series,
        )
    except _Unavailable as exc:
        return _unavailable("market", exc.code), None, None
    except VendorRateLimitError:
        raise
    except Exception:
        return _unavailable("market", "market_route_failure"), None, None


def _collect_news(request, routed, now):
    try:
        response = routed(
            "get_news", request.instrument, request.baseline_analysis_cutoff.isoformat(),
            request.analysis_cutoff.isoformat(), _provenance=True, _stop_on_rate_limit=True,
        )
        ...
        for match in _NEWS_ITEM.finditer(body):
            ...
            if available_at is not None:
                ...
                continue
            source = _news_source(content, sources)
            item = EvidenceItem.create(
                source=source.source, evidence_type="disclosure_or_news",
                requested_date=request.analysis_cutoff,
                effective_date=_effective_date(content), available_at=available_at,
                content=content, fallback=source.fallback,
                origins=(_pit_origin(source, "disclosure_or_news", _effective_date(content) or available_on),),
            )
            candidates.append(
                IncrementalEvidenceCandidate(
                    evidence=item,
        ...
        if not candidates:
            return (
                CollectionDomainResult(
                    domain="news", state=CollectionResultState.EMPTY, sources=sources,
                    diagnostic=CollectionDiagnostic(code="no_reliably_dated_japanese_records"),
                ),
                (),
            )
        return (
            CollectionDomainResult(
                domain="news", state=CollectionResultState.PARTIAL,
                sources=tuple(used_sources.values()), observed_from=min(observed), observed_through=max(observed),
        ...
        return _unavailable("news", exc.code), ()
    except VendorRateLimitError:
        raise
    except Exception:
        return _unavailable("news", "news_route_failure"), ()


def _collect_fundamentals(request, routed, now):
    try:
        response = routed(
            "get_fundamentals", request.instrument, request.analysis_cutoff.isoformat(),
...
def _market_series(request, source, body):
    ...
    if match is None or match.group("instrument").strip().casefold() != request.instrument.casefold():
    ...
    try:
        ...
        for row in rows:
            ...
            if not math.isfinite(value) or value <= 0 or session > request.analysis_cutoff or completed_at > request.window_end:
                omitted = True
                continue
            points.append(MarketSeriesPoint(session=session, completed_at=completed_at, adjusted_close=value))
    ...
    return MarketSeriesResult(
        instrument=request.instrument, source=source.source, fallback=source.fallback,
        adjustment_basis=basis, retrieved_at=source.retrieved_at, points=tuple(points),
    ), omitted
...
def _news_source(content, sources):
    name = (
        "EDINET" if ("filer:" in content or "Submitted:" in content)
        else "TDnet" if "Disclosed:" in content
        else "Google News"
    )
    source_id = _source_id(name)
    return next((source for source in sources if source.source == source_id), sources[0])
...
def _pit_origin(source, evidence_type, effective_date):
    return EvidenceOrigin(
        source=source.source, evidence_type=evidence_type,
        requested=effective_date.isoformat(), effective=effective_date.isoformat(),
        effective_date=effective_date, timing="market-date or publication-date filtered",
        retrieved_at=source.retrieved_at.isoformat().replace("+00:00", "Z"),
        fallback=source.fallback, temporal_scope="point_in_time",
    )


def _near_live_origin(source, evidence_type):
    return EvidenceOrigin(
        source=source.source, evidence_type=evidence_type, timing="live retrieval-time snapshot",
        retrieved_at=source.retrieved_at.isoformat().replace("+00:00", "Z"),
        fallback=source.fallback, temporal_scope="live_only",
    )


def _bounded_empty(domain, sources):
    return CollectionDomainResult(
        domain=domain, state=CollectionResultState.EMPTY, sources=sources,
...
def _unavailable(domain, code, *, sources=()):
    return CollectionDomainResult(
        domain=domain, state=CollectionResultState.UNAVAILABLE, sources=sources,
...
def _is_failure(body):
    return body.strip().casefold().startswith(("error fetching", "error retrieving", "error getting"))
