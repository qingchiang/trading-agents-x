...
def unsupported_crypto_base(raw: str) -> str | None:
    """Identify a Crypto-shaped pair so the public request can reject it.

    This helper does not normalize the symbol or make it vendor-routable.
    """
    ...
    if "-" in symbol:
        ...
        if (
            base.isalnum()
            and base
            and quote in _UNSUPPORTED_CRYPTO_QUOTES
        ):
            return base
    compact = symbol.replace("-", "")
    for quote in _UNSUPPORTED_CRYPTO_QUOTES:
